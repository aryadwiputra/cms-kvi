...
@section('content')
    <div class="card card-height-100 table-responsive">
        ...
        <div class="card-header border-bottom-dashed align-items-center d-flex">
            <h4 class="card-title mb-0 flex-grow-1">Destination</h4>
            <div class="flex-shrink-0">
                <a href="{{ route('destination.create') }}" class="btn btn-soft-primary btn-sm">
            ...
            <tbody>
                @forelse ($destinations as $data)
                    <tr>
                        ...
                        <td>
                            @if ($data->image)
                                <img src="{{ asset('destination_image/' . $data->image) }}" alt="" width="100px">
                            @else
                                No Image
                            @endif
                        </td>
                        ...
                        <td>
                            <div class="hstack gap-3 flex-wrap">
                                <a href="{{ route('destination.edit', $data->id) }}" class="link-success fs-15"><i
                                        class="ri-edit-2-line"></i></a>

                                <form id="delete-form-{{ $data->id }}"
                                    action="{{ route('destination.destroy', $data->id) }}" method="post">
                                    @csrf
                                    @method('delete')
                                    <a onclick="deleteData('{{ $data->id }}')" class="link-danger fs-15"><i
